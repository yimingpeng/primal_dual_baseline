    ...
    for problem in bullet_problems:
        ...
        for line in f:
            ...
            if "BipedalWalker-v2" in line:
                if algorithm == "DDPG":
                    line = "\t( python $pyName --env-id " + problem + "BulletEnv-v0" + " --seed $i  &> $i.out)\n"
                else:
                    line = "\t( python $pyName --env " + problem + "BulletEnv-v0" + " --seed $i  &> $i.out)\n"
            f1.write(line)
        f1.close()
        f.seek(0)
...
for algorithm in algorithms:
    for problem in gym_problems:
        ...
        for line in f:
            ...
            if "BipedalWalker-v2" in line:
                if algorithm == "DDPG":
                    line = "\t( python $pyName --env-id " + problem + "-v0" + " --seed $i &> $i.out)\n"
                else:
                    if problem == "LunarLanderContinuous" or problem == "BipedalWalker":
                        line = "\t( python $pyName --env " + problem + "-v2" + " --seed $i &> $i.out)\n"
                    else:
                        line = "\t( python $pyName --env " + problem + "-v0" + " --seed $i &> $i.out)\n"
            f1.write(line)
        f1.close()
        f.seek(0)
