...
def train(env_id, num_timesteps, seed):
    from baselines.dual_nac_advantage import mlp_policy, nac_simple
    ...
    nac_simple.learn(env, policy_fn,
            ...
            optim_epochs=1, optim_stepsize=0.05, optim_batchsize=64,
            gamma=0.99, lam=0.95,
            rho = 0.95,  # Gradient weighting factor
            update_step_threshold = 30, # Updating step threshold
                     shift = 0,
                     schedule='linear'
        )
    env.close()
...
def main():
    ...
    logger.configure(
                     format_strs=['stdout', 'log', 'csv'], log_suffix = "Dual-NAC-Advantage-"+args.env)
    logger.log("Algorithm: Dual-NAC-Advantage-"+args.env)
